...
wb2.xlsx.readFile("./second.xlsx").then(function(){
    ...
    function compareTwoSheets(sh1:any, sh2:any){
        var firstRow1:number = sh1.getRow(1);
            var lastRow1: number = sh1.getRow(sh1.rowCount);
            var equalSheets: boolean = true;
            for(let i=firstRow1; i <= lastRow1; i++) {
    ...
    function compareTwoRows(row1:any,row2:any){
        ...
        var equalRows: boolean = true;
        
        // Compare all cells in a row
        for (let i = 1; i <= sh1.cellCount; i++) {
    ...
    function compareTwoCells(cell1:any, cell2:any) {
